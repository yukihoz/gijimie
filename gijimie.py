...
st.title(':thinking_face: 議会の議事録検索@東京都中央区')
st.subheader('「あの話題、どの議員さんが熱心に活動してるのかな？？」と思ったときに。')
st.markdown('　区議会のwebサイトでもキーワードでの検索は可能なんですが会議体ごとで表示されて見にくいので、関連する部分だけを抜き出して表示するサービスを作ってみました。')
...
st.markdown('キーワードが含まれる発言内容の文字列をカウントして、ランキング化したものです。ざっくりどの議員がそのテーマに熱心なのかを測るのに使えるかも。')
st.markdown('カテゴリとしては以下の3つに分かれています「質問」「回答」キーワードが含まれる発言内容の文字列をカウントして、ランキング化したものです。')
st.markdown('「質問」：議員による質問内容、「回答」：議員の質問に対する区長などの回答、「議長/委員長」：会議での議長や委員長としての発言')
logs_contents_temp_moji = selected_l_moji.groupby(['人分類','内容分類'], as_index=False).sum()# 年度ごとの文字数
#st.bar_chart(logs_contents_temp_moji)
fig = px.bar(logs_contents_temp_moji, x='文字数', y='人分類', color='内容分類', text='文字数',height=800, width=1000, orientation='h')
...
st.markdown('キーワードが含まれる発言内容のテキストを解析して、ざっくり1枚の画像にしてます。そのキーワードでどういう議論をされているのかを掴むのに役立つかも。')
# ワードクラウド作成
...
wc = WordCloud(stopwords=stpwds, width=1080, height=1080, background_color='white', colormap='Dark2', font_path = font_path)
wc.generate(words)
wc.to_file('wc.png')
st.image('wc.png')
